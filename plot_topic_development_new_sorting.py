...
for c in econ_colors + tech_colors + gov_colors + sci_colors + com_colors + other_colors:
    color_map.append([c[0]/255, c[1]/255, c[2]/255, 0.15])
...
for loop_index, el in enumerate(topics_manually_sorted):
    ...
    if (loop_index) % TOPICS_PER_ROW == 0:
        #jump to next row
        ...
        current_x = 0.03
        plt.axis('off')
        if loop_index == 0: # only for first row
    ...
    ax3.fill([background_x_start, background_x_end, background_x_end, background_x_start, background_x_start], [background_y_start, background_y_start, background_y_end, background_y_end, background_y_start], color = color_map[loop_index])

    topic_description_font_size = 3.3
    topic_description_extra_jump = 1.2
    MAX_DESC_LEN = 37
    topic_desc = topic_name_dict[str(id)]
    string_list = []
    current_string = ""
